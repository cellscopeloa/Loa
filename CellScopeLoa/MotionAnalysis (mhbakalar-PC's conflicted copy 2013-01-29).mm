...
{    
    ...
    int bufferIdx = movieIdx*numFrames + frameIdx;
...
- (void)nextMovie {
    NSLog(@"%d frames in last movie", frameIdx);
    movieIdx++;
    frameIdx = 0;
}

- (void)processMovies {
    // Start at the first movie, first frame
    frameIdx = 0;
    movieIdx = 0;
    ...
    while(i < 115/framesToSkip) {
        while(i <= avgFrames) {
            // Grab the current movie from the frame buffer list
            int bufferIdx = movieIdx*numFrames + (frameIdx+15);
            movieFrameMat = frameBuffers->at(bufferIdx);
        ...
        if (i > avgFrames) {
            ...
            // Grab the current movie from the frame buffer list
            bufferIdx = movieIdx*numFrames + (frameIdx+15);
